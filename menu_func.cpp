...
void start_menu(SDL_Renderer* ren) {
	...
	while (isRunning) {
		...
		if (isEnter) {
			switch (menuChoice) {
				case 1: if(level_menu(ren)) isRunning = false; break;
				case 2: break;
...
int level_menu(SDL_Renderer* ren) {
	...
	TTF_Font* font = TTF_OpenFont("fonts/CherryBombOne-Regular.ttf", 70);
	const char *menuPointer = "-->";
	const char *buttonStart = "LEVEL 1";
	const char *buttonCreateLevel = "LEVEL 2";
	const char *buttonAbout = "LEVEL 3";
	...
	while (isRunning) {
		...
		if (isEnter) {
			switch (menuChoice) {
				case 1: level1_map_mask(ren); return 1; break;
				case 2: level2_map_mask(ren); return 1; break;
				case 3: level3_map_mask(ren); return 1; break;
